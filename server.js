...
const requireLogin = (req,res,next) => { if(!req.session?.user) return res.status(401).json({error:"로그인 필요"}); next(); };
const requireAdmin = (req,res,next) => { if(!req.session?.isAdmin) return res.status(403).json({error:"관리자 인증 필요"}); next(); };
...
app.post("/api/logout",(req,res)=>{
  req.session.destroy(err=> err ? res.status(500).json({error:"로그아웃 실패"}) : res.json({success:true}) );
});

// --- 게시글 목록
app.get("/api/posts",(req,res)=>{
  const posts = (db.data.posts || []).slice();
  // 랜덤으로 섞기
