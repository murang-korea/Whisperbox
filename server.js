...
app.post("/api/signup", async (req, res) => {
  ...
  if (!username || !password || !nickname)
    return res.status(400).json({ error: "모든 항목 입력" });
...
app.post("/api/login", (req, res) => {
  ...
  if (!username || !password)
    return res.status(400).json({ error: "아이디/비밀번호 입력" });
...
app.post("/api/posts", requireLogin, async (req, res) => {
  ...
  if (!title || !content)
    return res.status(400).json({ error: "제목과 내용 입력" });
...
app.delete("/api/posts/:id", requireLogin, async (req, res) => {
  ...
  if (!isAuthor && !req.session.isAdmin)
...
app.put("/api/posts/:id", requireLogin, async (req, res) => {
  ...
  if (req.session.user.username !== post.authorUsername)
